...
public:
    ...
    Scalar volume() const
    {
        if (BoundaryCellManager<Scalar>::getInstance().isBoundaryCell(elementIndex_)) {
            auto volume = BoundaryCellManager<Scalar>::getInstance().boundaryCellVolume(elementIndex_);
            // std::cout << "Boundary cell found with volume " << volume << std::endl;
